...
namespace System
{
        ...
        , IComparable<BigDecimal>
    {
        ...
        /// <summary>
        /// Converts the string representation of a decimal number to its BigDecimal equivalent.
        /// Supports plain decimals ("123.456", "-0.001") and scientific notation ("1.23E+4", "5E-10").
        /// </summary>
        /// <exception cref="FormatException">Thrown when the string is not a valid number.</exception>
        public static BigDecimal Parse(string value)
        {
            if (!TryParse(value, out var result))
                throw new FormatException($"'{value}' is not a valid BigDecimal.");
            return result;
        }

        /// <summary>
        /// Tries to convert a string to a BigDecimal. Returns false without throwing if the format is invalid.
        /// Supports plain decimals ("123.456", "-0.001") and scientific notation ("1.23E+4", "5E-10").
        /// </summary>
        public static bool TryParse(string value, out BigDecimal result)
        {
            ...
            // Match optional sign, integer digits, optional fractional part, optional exponent
            // Examples: "123", "-0.456", "1.23E+4", "5.6e-10", ".5", "-.5"
            var match = Regex.Match(value,
                @"^(?<sign>[+-]?)(?<int>\d*)(?:\.(?<frac>\d*))?(?:[eE](?<exp>[+-]?\d+))?$");
            ...
            // Combine integer and fractional digits into one BigInteger mantissa
            string digits = intPart + fracPart;
            if (!BigInteger.TryParse(digits, out BigInteger mantissa))
                return false;

            if (negative)
                mantissa = -mantissa;

            // The fractional part shifts the exponent left; the explicit exponent shifts it further
            int exponent = -fracPart.Length;
            if (expPart.Length > 0)
            {
                if (!int.TryParse(expPart, out int parsedExp))
                    return false;
                exponent += parsedExp;
            }
        ...
        public static implicit operator BigDecimal(string value) => Parse(value);
